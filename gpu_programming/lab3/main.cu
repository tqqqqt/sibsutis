...
__global__ void TestFunction(int *masA, int *masB){
...
int main(int argc, char **argv){
	if(argc<2){
		fprintf(stdout,"Need 2 argum - ./prog <nCoreInBlock>.\n");
		return 4;
	}
	int nn=1<<20, nCoreInBlock=atoi(argv[1]);
	...
	TestFunction<<<dim3(nBlock),dim3(nCoreInBlock)>>>(masA,masB);
	cudaDeviceSynchronize();
	cudaEventRecord(stop,0);
	cudaEventSynchronize(stop);
	cudaEventElapsedTime(&elapsedTime,start,stop);
	fprintf(stdout,"Elapsed time - %g\n",elapsedTime);
	
	cudaMemcpy(mas,masA,nn*sizeof(int),cudaMemcpyDeviceToHost);
	for(i=0;i<nn;i+=nn/8) fprintf(stdout,"%d\t%d\n",i,mas[i]);
