...
PNum_Converter::PNum_Converter(){ }

std::string PNum_Converter::convertFrom10(double num, int p, int c){
    std::string result="";
    if(num<0){
        result+='-';
        num=std::abs(num);
...
double PNum_Converter::convertTo10(std::string p_num, int p){
    double result=0;
    short minus_flg=1;
    if(p_num[0]=='-'){
        minus_flg=-1;
...
// convert from 10
char PNum_Converter::intToChar(int num){
    if(num>=0 && num<=9) return ('0'+num);
    num-=10;
    return ('A'+num);
}

std::string PNum_Converter::intToP(int num, int p){
    if(num==0) return "0";
...
std::string PNum_Converter::fltToP(double num, int p, int c){
    std::string result="";
...
// convert to 10
int PNum_Converter::charToInt(char num){
    if(num>='0' && num<='9') return (num-'0');
    return 10+(num-'A');
}

double PNum_Converter::strToFlt(std::string num, int p, double width){
    double result=0;
    int temp_num=0;
