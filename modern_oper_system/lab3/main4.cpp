...
int chanel2=ChannelCreate(0);
std::vector<int> cords;

int nd=0, pid=0, chid=0;
...
int main(){
	nd=netmgr_strtond("EA43e49d",0);
	int fd=open("/net/EA43e49d/dev/shmem/gener",O_RDWR);
	read(fd,&pid,4);
	read(fd,&chid,4);
	std::cout<<"nd= "<<nd<<", pid="<<pid<<", chid="<<chid<<'\n';
	...
	pthread_t pt_waiter, pt_child1, pt_child2, pt_child3, pt_child4, pt_tran, pt_reader;
	...
	int coid_chanel=ConnectAttach(nd,pid,chid,0,0);
	if(coid_chanel==-1){
		std::cout<<"Error connect to chanel.\n";
		return -1;
	}

	MoveTo(30,272,elips);
	int curent_grab_type=0, curent_point=4, curent_grab_power=0;
	int temp=2423, temp_ans=-1, temp_buf=0, ttemp_buf=10;
	while(!exit_flag){
		pthread_mutex_lock(&mutex_que);
		if(load_que.size()!=0){
			temp=MsgSend(coid_chanel,&ttemp_buf,sizeof(int),&temp_buf,sizeof(int));
			if(temp_buf==-1){
				pthread_mutex_unlock(&mutex_que);
				continue;
			}
			curent_grab_power=temp_buf;
		}
		else{
			pthread_mutex_unlock(&mutex_que);
			continue;
...
void* second_tran(void* args){
	...
	MoveTo(15,272,elips);
	int curent_grab_type=0, curent_point=4, curent_grab_power=0;
	int temp=-1, temp_ans=-1, temp_buf=0, ttemp_buf=10;
	while(!*arg->exit_flag){
		pthread_mutex_lock(&mutex_que);
		if(load_que.size()!=0){
			temp=MsgSend(coid_chanel,&ttemp_buf,sizeof(int),&temp_buf,sizeof(int));
