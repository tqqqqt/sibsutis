...
int main(){
	...
	argums *arg_5=new argums(0,0,0,0,0,&exit_flag);
	
	pthread_t pt_waiter, pt_child1, pt_child2, pt_child3, pt_child4, pt_generator;
	...
	while(!exit_flag){
		...
		if(load_que.size()==0) MsgReply(temp,0,&temp_ans,sizeof(int));
			...
			temp_buf=curent_grab_power>10?curent_grab_power-10:0;
			MsgReply(temp,0,&temp_buf,sizeof(int));
		}
		if(exit_flag) break;
		if(load_que.size()==0) continue;
	...
	MsgSendPulse(coid_chanel,10,10,10);
	CloseGraph();
