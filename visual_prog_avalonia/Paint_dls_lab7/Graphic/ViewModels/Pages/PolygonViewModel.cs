...
namespace Graphic.ViewModels.Pages
{
    public class PolygonViewModel : ViewModelBase
    {
        ...
        public PolygonViewModel(ref ObservableCollection<IFigure> col)
        {
            colection = col;
            name = string.Empty;
            points = string.Empty;
        ...
        public void ChangeOption(int num)
        {
            ...
            var figure = colection[select_figure] as Gr_Polygon;
            Name = figure.Name;
        ...
        public void Button_add()
        {
            if (Points != null && Name != null)
            {
                ...
                Graphic.Models.Gr_Polygon polygon = new Gr_Polygon(Name, temp_all_point, color11, Thic, color22);
                polygon.Gr_Polygon_transform(Angle, Rotate, Scale, Skew);
                if (flag == 0) colection.Add(polygon);
                else if (flag == 1)
                {
                    colection[select_figure] = polygon;
                    flag = 0;
                    select_figure = -2;
                }
                Button_cancel();
