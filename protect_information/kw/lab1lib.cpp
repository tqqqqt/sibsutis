...
bool checkPrime(std::string num){
	if(MaxNumber(num,"1")>=0) return false;
	if(MaxNumber(num,"2")==0) return true;
	std::string a=std::to_string(rand()%(stoll(num)-1)+2);
	if(findModule(a,stoll(num)-1,num) != std::get<0>(findNod(num,a))) return false;
	return true;
}