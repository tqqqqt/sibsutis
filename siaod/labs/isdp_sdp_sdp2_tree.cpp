...
}*root_isdp=NULL, *root_sdp=NULL, *root_sdp2=NULL;
...
int main(){
	...
	check(mas,mas_check);
	for(int i=lef;i<last_el;i++){ sdp(mas[i],root_sdp); sdp_v2(mas[i],root_sdp2); }
	quick(mas,lef,riht);
...
//	tree_print(root_isdp);
	...
	float h5=sred_d_p(root_sdp2,1), h6=tree_size(root_sdp2), sred_h3=h5/h6;
	...
	printf(" SDP2 %6d %12d %12d %12.2f\n",tree_size(root_sdp2),tree_sum(root_sdp2),tree_height(root_sdp2),sred_h3);
