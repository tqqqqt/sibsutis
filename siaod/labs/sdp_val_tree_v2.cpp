...
int main(){
	...
	struct mas che[101];
	int ma[100];
	for(int i=0;i<101;i++){ che[i].data=i; che[i].check=0; }
	for(int i=0;i<100;i++) ma[i]=rand()%100+1;
	check(ma,che);
	for(int i=0;i<100;i++) add(ma[i],avl_root);
	for(int i=0;i<100;i++) sdp(ma[i],root_sdp);
	prin(avl_root);
	printf("\n \n");
